...
class BSDF(Material):
    ...
    def from_values(cls, values_dict):
        ...
        if values_dict['type'] != 'BSDF':
            raise ValueError(
                'Type must be %s not %s.' % ('BSDF', values_dict['type'])
            )

        modifier, dependencies = cls.filter_dict_input(values_dict)
        values = values_dict['values'][0]
        extra_values = values_dict['values'][2]
    ...
    @classmethod
    def from_dict(cls, input_dict, folder='.'):
        """Make radiance material from dict.

        Provide dictionary and destination folder to save the bsdf file.
        ...
        if input_dict['type'] != 'BSDF':
            raise ValueError(
                'Type must be %s not %s.' % ('BSDF', input_dict['type'])
            )
        ...
        if 'front_diffuse_reflectance' in input_dict:
            cls_.front_diffuse_reflectance = input_dict['front_diffuse_reflectance']
            if 'back_diffuse_reflectance' in input_dict:
                cls_.back_diffuse_reflectance = input_dict['back_diffuse_reflectance']
                if 'diffuse_transmittance' in input_dict:
                    cls_.diffuse_transmittance = input_dict['diffuse_transmittance']
